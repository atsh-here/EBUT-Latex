...
\begin{enumerate}[leftmargin=*]
  \item \textbf{ZK-membership with leaf nullifier (recommended for high-privacy deployments).}
    The server publishes a Merkle root $\mathsf{rt}$ of a set of nonces. The client selects a leaf $\eta_i$ and its inclusion path \emph{offline} (e.g., from a previously cached full tree or via an anonymous channel). When spending, instead of revealing $\eta_i$ and its path, the client proves in zero-knowledge that $\eta_i$ is a member of the tree under $\mathsf{rt}$, and derives a deterministic leaf nullifier $N_\eta = \mathcal{H}_{\mathbb{G}_1}(\text{``EBUT:LeafNullifier:''} \parallel \eta_i)$ which is submitted in place of $\eta$. The server checks $N_\eta \notin \mathcal{N}_{\mathsf{spent}}$ and verifies the ZK membership proof against $\mathsf{rt}$, then marks $N_\eta$ as spent. This completely decouples the identity of the leaf from the spend submission. The membership proof can be constructed using a standard Merkle-Schnorr ZK argument or a succinct proof system (e.g., a Merkle proof inside a Bulletproof/STARK circuit).
