...
\item \textbf{Nullifier freshness check (read-only):} $N_T \notin \DB_{\mathsf{epoch}}$ and $N_T \neq 1_{\mathbb{G}_1}$. \textbf{Do not insert $N_T$ at this step.} Inserting before the proofs are verified creates a TOCTOU vulnerability: an adversary can submit a valid $N_T$ with garbage ZK proofs, causing the server to burn the legitimate user's epoch nullifier in step~2 and then abort during proof verification, permanently locking the user out for the entire epoch.
...
\item \textbf{Pairing check:}
  ...
  \]
\item \textbf{Atomic state mutation (step 10, LAST):} Only after all checks in steps~1--9 pass: atomically insert $N_T$ into $\DB_{\mathsf{epoch}}$ (reject if already present, indicating a concurrent duplicate or replay). This deferred write ensures that a request with a valid $N_T$ but invalid proofs cannot permanently burn the epoch nullifier of a legitimate user.
\end{enumerate}
...
\item \textbf{Responses:}
  Use the $z = u_m + c \cdot m$ accepted in step~1 (over the integers, not reduced modulo any group order).
