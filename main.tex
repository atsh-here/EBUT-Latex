...
\begin{itemize}
\item $\DB_{\mathsf{epoch}}$: stores epoch nullifiers $N_T \in \mathbb{G}_1$ observed during $\mathsf{Refresh}$; cleared at the start of each epoch.
\item $\DB_{\mathsf{spend}}$: stores token nullifiers $k_{\mathsf{cur}} \in \mathbb{Z}_q^*$ observed during $\mathsf{Spend}$; cleared at the start of each epoch.
\end{itemize}
...
\item A context hash $H_{\mathsf{ctx}}$ that binds the protocol instance to an application identifier and the public keys, preventing cross-deployment replay. Specifically,
  ...
  \]
  where $\mathcal{H}_{\mathsf{scalar}}: \{0,1\}^* \to \mathbb{Z}_q$ is a hash function modeled as a random oracle.
\end{itemize}
...
\item \textbf{Nullifier freshness:} $N_T \notin \DB_{\mathsf{epoch}}$ and $N_T \neq 1_{\mathbb{G}_1}$. Atomically insert $N_T$ if fresh.
...
\]
It caches the response for idempotency and sends $(A_{\mathsf{daily}}, e_{\mathsf{daily}}, s'_{\mathsf{daily}})$ to the client.
...
\item \textbf{Commitments.}
  ...
  \begin{align*}
    ...
    T_{\mathsf{scale},R} &= \rho_{r_1} \cdot K', & T_{\mathsf{refund}} &= \rho_m \cdot h_4 + \rho_u \cdot h_1 + \rho_{r_1} \cdot T \cdot h_2 + \rho_v \cdot h_0, \\
...
\item \textbf{Challenge.}
  ...
  A' \parallel \bar{A} \parallel T_{\mathsf{BBS}} \parallel T_{\mathsf{scale},T} \parallel T_{\mathsf{total}} \parallel T_{\mathsf{scale},R} \parallel T_{\mathsf{refund}} \parallel T_{\mathsf{scale},BP} \parallel T_{\mathsf{BP}} \parallel \text{``Spend''}
  \end{array}
  \right).
  \]
\item \textbf{Responses.}
...
\item \textbf{Idempotency cache check (FIRST):} Compute $h_{\mathsf{id}} = \mathcal{H}_{\mathsf{scalar}}(\pi_{\mathsf{spend}} \parallel \eta)$. If $h_{\mathsf{id}}$ is in the idempotency cache, return the cached refund token immediately \textbf{without checking the double-spend database}.
\item \textbf{Nonce freshness:} Verify that $\eta$ is valid and has not been used before. In standard mode, check a short-lived cache; in high-privacy mode, verify a Merkle inclusion proof and check a Bloom filter.
\item \textbf{Double-spend check (SECOND):} Atomically check that $k_{\mathsf{cur}} \notin \DB_{\mathsf{spend}}$. \textbf{Reject if $k_{\mathsf{cur}} = 0$}.
...
\item \textbf{Verify bridging equalities:}
  \begin{align*}
    ...
    z_m \cdot h_4 + z_u \cdot h_1 + z_{r_1} \cdot T \cdot h_2 + z_v \cdot h_0 - T_{\mathsf{refund}} &\stackrel{?}{=} z_{r_1} \cdot K' - T_{\mathsf{scale},R}, \\
...
If all checks pass, the server issues a refund token by choosing $e^*, s'^* \xleftarrow{\$} \mathbb{Z}_q$ and computing
\[
A^* = \left( g_1 \cdot K' \cdot h_0^{s'^*} \right)^{\frac{1}{e^* + y}}.
\]
It caches the response for idempotency and sends $(A^*, e^*, s'^*)$ to the client.
...
\item \textbf{Range Proof:}
  Generate a Dalek Bulletproof $\pi_{\text{range}}$ for the Ristretto commitment $C_{25519}$ proving that $m \in [0, 2^{32}-1]$. The Bulletproof transcript is initialized with domain separator \texttt{"EBUT:BEQ:Range"} and includes $\text{ctx}$ and the serialized auxiliary commitments $\text{comm}_i$.

\item \textbf{Challenge:}
  Compute the 128-bit challenge
  \[
  c = \mathcal{H}_{128}\bigl(\text{ctx} \parallel C_{\text{BLS}} \parallel C_{25519} \parallel R_{\text{BLS}} \parallel R_{25519} \parallel \pi_{\text{range}} \parallel \text{comm}_1 \parallel \dots \parallel \text{comm}_k \bigr).
  \]

\item \textbf{Responses:}
  Compute over the integers:
  \[
  z = u_m + c \cdot m \quad (\text{as an integer, not reduced modulo any group order}).
  \]
...
\item \textbf{Bound Check:}
  Verify that $0 \le z < q_{25519}$ (as an integer). If not, reject.

\item \textbf{Recompute Challenge:}
  Compute $c = \mathcal{H}_{128}(\text{ctx} \parallel C_{\text{BLS}} \parallel C_{25519} \parallel R_{\text{BLS}} \parallel R_{25519} \parallel \pi_{\text{range}} \parallel \text{comm}_1 \parallel \dots)$.
...
\end{theorem}
\begin{proof}
The integer $z = u_m + c \cdot m$ satisfies $0 \le z < 2^{241} < q_{25519}$ by construction, so the bound check passes. Substituting the definitions:
\[
...
Under the random oracle model, the non-interactive BEQ protocol is statistically zero-knowledge with distinguishing advantage bounded by $2^{-80}$.
\end{theorem}
\begin{proof}
...
\item Choose random $z \xleftarrow{\$} [0, 2^{241}-1]$, $z_{\text{BLS}} \xleftarrow{\$} \mathbb{Z}_{q_{\text{BLS}}}$, $z_{25519} \xleftarrow{\$} \mathbb{Z}_{q_{25519}}$.
...
\end{enumerate}
In a real execution, $z = u_m + c \cdot m$ where $u_m \in [0,2^{240}-1]$ and $c \cdot m < 2^{160}$. Thus $z$ is uniformly distributed over an interval of length $2^{240}$ shifted by at most $2^{160}$. The simulated $z$ is uniform over $[0, 2^{241}-1]$. The statistical distance between these two distributions is bounded by $\frac{2^{160}}{2^{240}} = 2^{-80}$, which is negligible. The other components are identically distributed. Hence, the simulation is statistically indistinguishable.
\end{proof}
...
When the server issues a Daily Token, it computes a cache key:
\[
\mathtt{key}_{\mathsf{ref}} = \mathcal{H}_{\mathsf{scalar}}(\pi_{\mathsf{refresh}} \parallel \text{``Idempotency''}),
\]
and stores the mapping $\mathtt{key}_{\mathsf{ref}} \mapsto (A_{\mathsf{daily}}, e_{\mathsf{daily}}, s'_{\mathsf{daily}})$. The cache entry should persist for the remainder of the epoch (or at least several minutes). If a client resubmits an identical $\pi_{\mathsf{refresh}}$, the server returns the cached response \textbf{without} re-executing the expensive verification (pairings, MSMs) and \textbf{without} re-checking the epoch nullifier in $\DB_{\mathsf{epoch}}$.

\paragraph{Spend Phase Idempotency.}
For spend requests, the cache key is:
\[
h_{\mathsf{id}} = \mathcal{H}_{\mathsf{scalar}}(\pi_{\mathsf{spend}} \parallel \eta).
\]
The server stores $h_{\mathsf{id}} \mapsto (A^*, e^*, s'^*)$ for a short duration (e.g., 60 seconds). \textbf{Crucially, the idempotency cache must be checked \emph{before} the double-spend database.} If a match is found, the cached refund token is returned immediately. This prevents legitimate retries from being incorrectly flagged as double spends.
...
\begin{itemize}
\item $\DB_{\mathsf{epoch}}$: stores compressed $\mathbb{G}_1$ points (48 bytes). Cleared at the start of each epoch.
\item $\DB_{\mathsf{spend}}$: stores canonical scalars (32 bytes). Cleared at the start of each epoch.
\end{itemize}

\paragraph{Epoch Grace Period.}
The server must keep the previous epoch's $\DB_{\mathsf{spend}}$ for a grace period (e.g., 5 minutes) after the epoch boundary. Spend requests with $T_{\mathsf{issue}} = T_{\mathsf{current}} - 1$ during this window are checked against the previous epoch's database. This accommodates clients with slight clock skew or in-flight requests near the boundary.
...
A user may legitimately generate up to $c_{\mathsf{max}}$ spends in a short period. To handle this efficiently without scanning the entire database, the server should use a \textbf{Bloom filter} or \textbf{Cuckoo filter} in front of the main $\DB_{\mathsf{spend}}$ store. The filter is cleared with the database at epoch boundaries. This provides fast, constant-time rejection of duplicates and mitigates resource exhaustion from flood attacks.

\subsection{Verification Order (Optimized)}

To prevent CPU exhaustion attacks from malformed proofs, the server must perform checks in the following order (fastest to slowest):

\begin{enumerate}[leftmargin=*]
\item Epoch and parameter bounds (e.g., $s > 0$, $T$ valid).
\item \textbf{Idempotency cache check} (return cached token if hit).
\item Nonce freshness (and Merkle proof if applicable).
\item Nullifier freshness (atomic DB check) and zero-scalar rejection ($N_T \neq 1_{\mathbb{G}_1}$, $k_{\mathsf{cur}} \neq 0$).
...
The standard nonce acquisition method (\texttt{GET /nonce}) creates a temporal correlation between the client's IP address and the subsequent spend request, breaking anonymity at the network layer. For deployments where transport-layer privacy is critical, implementers \textbf{must} use the high-privacy nonce distribution method: the server periodically publishes a Merkle root of pre-generated nonces. The client selects a nonce, obtains its Merkle inclusion proof, and submits the proof with the spend request. The server verifies the proof and marks the nonce as spent (e.g., in a Bloom filter). This decouples nonce acquisition from spend submission. Because Bloom filters can yield false positives, the client SDK must be prepared to retry with a different nonce upon receiving an HTTP 409 Conflict.
