...
\item A context hash $H_{\mathsf{ctx}}$ that binds the protocol instance to an application identifier and the public keys, preventing cross-deployment replay. Specifically,
  ...
  \]
  where $\mathcal{H}_{\mathsf{scalar}}: \{0,1\}^* \to \mathbb{Z}_q$ is a hash function modeled as a random oracle. \textbf{Implementation requirement:} Because $q$ (the BLS12-381 scalar field order) is a 255-bit prime, a direct reduction of a 256-bit hash output modulo $q$ introduces severe statistical bias — values in the lower half of $\mathbb{Z}_q$ are approximately twice as likely as values in the upper half, breaking the zero-knowledge property. To prevent modulo bias, $\mathcal{H}_{\mathsf{scalar}}$ \textbf{must} be implemented by first generating a digest significantly wider than $q$, then reducing. Concretely, either (a) hash to at least 64 bytes using SHA-512 (or BLAKE2b-512) before reducing modulo $q$, or (b) use the \texttt{expand\_message\_xmd} function from RFC 9380 with sufficient output length. Raw \texttt{SHA-256(data) mod q} is explicitly \textbf{prohibited}.
\end{itemize}
...
\item \textbf{Idempotency cache check (FIRST):} Compute $h_{\mathsf{id}} = \mathcal{H}_{\mathsf{scalar}}(\text{``EBUT:Idempotency''} \parallel \mathsf{len}(\pi_{\mathsf{spend}}) \parallel \pi_{\mathsf{spend}} \parallel \eta)$. If $h_{\mathsf{id}}$ is in the idempotency cache, return the cached refund token immediately \textbf{without checking the double-spend database}.
\item \textbf{Nonce read-only check:} Verify that $\eta$ is structurally valid and has not yet been marked spent. In standard mode, check a short-lived cache; in high-privacy mode, verify a Merkle inclusion proof and query the exact nonce store. \textbf{Do not mark $\eta$ as spent at this step.}
...
\item \textbf{Atomic state mutation (LAST):} Only after all checks in steps 1--13 pass: atomically insert $k_{\mathsf{cur}}$ into $\DB_{\mathsf{spend}}$ (reject if already present, indicating a concurrent double-spend race); atomically mark $\eta$ as spent in the nonce store. These writes must be performed as a single atomic transaction or with optimistic concurrency control to prevent TOCTOU races.
\end{enumerate}
...
\item \textbf{Challenge:}
  ...
  \[
  c = \mathcal{H}_{128}\bigl(\text{``EBUT:BEQ:Challenge''} \parallel \text{ctx} \parallel C_{\text{BLS}} \parallel C_{25519} \parallel R_{\text{BLS}} \parallel R_{25519} \parallel \pi_{\text{range}} \parallel \text{comm}_1 \parallel \dots \parallel \text{comm}_k \bigr).
  \]
...
\item \textbf{Recompute Challenge:}
  Compute $c = \mathcal{H}_{128}(\text{``EBUT:BEQ:Challenge''} \parallel \text{ctx} \parallel C_{\text{BLS}} \parallel C_{25519} \parallel R_{\text{BLS}} \parallel R_{25519} \parallel \pi_{\text{range}} \parallel \text{comm}_1 \parallel \dots)$.
...
\item \textbf{Atomic state mutation} (insert nullifier into $\DB_{\mathsf{spend}}$, mark nonce as spent; reject on concurrent duplicate).
\end{enumerate}
...
The standard nonce acquisition method (\texttt{GET /nonce}) creates a temporal correlation between the client's IP address and the subsequent spend request, breaking anonymity at the network layer. For deployments where transport-layer privacy is critical, implementers \textbf{must} use the high-privacy nonce distribution method: the server periodically publishes a Merkle root of pre-generated nonces. The client selects a nonce, obtains its Merkle inclusion proof, and submits the proof with the spend request. The server verifies the proof and, once all cryptographic checks pass, marks the nonce as spent in an \textbf{exact, authoritative store} (e.g., a Redis \texttt{SADD} with TTL-based expiry). This decouples nonce acquisition from spend submission. A probabilistic structure such as a Bloom filter \textbf{must not} be used as the authoritative record of spent nonces: its false-positive rate rises as the epoch progresses, causing legitimate spends to be incorrectly rejected with HTTP 409 Conflict responses. A Bloom filter may be used as a fast-reject layer in front of the authoritative store, but any positive result must be confirmed with an exact lookup before the request is rejected.
