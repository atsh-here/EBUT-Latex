...
\item \textbf{Nonce read-only check:} Verify that $\eta$ is non-zero, is exactly 16 bytes, and has not yet been marked spent. In standard mode, check a short-lived cache; in high-privacy mode, verify a Merkle inclusion proof and query the exact nonce store. \textbf{Do not mark $\eta$ as spent at this step.}
...
\item \textbf{Atomic state mutation (step 14, LAST):} Only after all checks in steps 1--13 pass: atomically insert $k_{\mathsf{cur}}$ into $\DB_{\mathsf{spend}}$ (reject if already present, indicating a concurrent double-spend race); atomically mark $\eta$ as spent in the nonce store. These writes must be performed as a single atomic transaction or with optimistic concurrency control to prevent TOCTOU races.
\end{enumerate}
